...
The strongest available check is external, and it is the one this appendix rests
on. Ohta reports a mean $L^{S0}$ of 0.106 and a mean $L^{S0C}$ of 0.806 across
2010--2022, computed from the same kind of feed by an independent implementation.

\input{chapters/_ladder_validation}

Agreement on the submission-share levels is not something a broken
implementation produces by accident: the two rules in
Section~\ref{app:ladder} --- matching by price and requiring observability in
both snapshots --- are exactly what stands between this and a number several
times too large. The cancellation-ratio overshoot is the honest residual of
the exercise. It has two live explanations --- cancellation traffic genuinely
rose between the paper's sample and this one, or the netting inside
ten-level intervals biases the ratio upward here --- and nothing in this
study distinguishes them, so the ratio is not claimed as validated.
