...
liquidity} did not already say? With 78 time periods the resulting
...
0.0192, significant at the 1\% level. Read economically, a one-standard-deviation
...
coefficient is
0.0432, not statistically distinguishable from zero.
...
\paragraph{Verdict.}
Clustering carries information about next-day liquidity that the outcome's own history does not, in a specification demanding enough to absorb every stock-level and market-wide effect. The association is descriptive: no experiment, and no claim that intervening on clustering would move spreads.
