...
That choice has a cost worth naming. Four consecutive months is one market regime, not a cross-section of them, and this particular block is not a quiet one: April 2025 delivered almost twice January's message volume. The half-sample split in Section~\ref{sec:robust} is doing real work here rather than serving as a formality. Nothing about the pipeline changes with more months --- the same command extends it and the report regenerates from whatever the panel contains --- but the results below describe this stretch of 2025, not the year.
