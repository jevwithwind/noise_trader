...
\midrule
1 yen & 875 & kept \\
5 yen & 39 & excluded by filter (d) \\
10 yen & 37 & kept \\
0.1 yen & 36 & kept \\
0.5 yen & 34 & excluded by filter (d) \\
\bottomrule
\end{tabular}
\end{table}
...
1,024 stock-days attempted, 880
passing all four filters, and 850 surviving the requirement that a stock
qualify on more than half the sample --- 425 stocks over 2
trading days. The full waterfall is reported in the stage output; no stock-day
disappears without being counted.

\textbf{Coverage.} The panel covers 2 trading days across 1 calendar month(s), from 2024-01-04 to 2024-01-05, rather than the full 240-day year. Ingesting the complete tape for this universe takes substantially longer than the time available for this prototype, so months were ingested in an order that spreads them across the calendar rather than concentrating them at its start. Every result below should be read as covering that sample. Nothing about the pipeline changes with more months: the same command extends it, and the report regenerates from whatever the panel contains.
