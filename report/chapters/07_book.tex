...
positive: it is 0.0029, not statistically distinguishable from zero. Orders that are \emph{not}
cancelled are the ones that get picked off, so the coefficient on the
cancellation ratio should be negative: it is -0.0092, not statistically distinguishable from zero.
And more standing round-price inventory should mean more round-price execution:
the coefficient on $\mathit{RDepth}$ is 0.2565, significant at the 1\% level.
...
interaction term is 0.0514, not statistically distinguishable from zero. A positive value means
